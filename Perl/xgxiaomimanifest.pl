...
foreach my $key (keys %$xmlData)
{
    ...
    if ($key eq "uses-permission") {
        ...
        {
            $count = @{$serviceData};
            my @xgForceServiceList = ("com.xiaomi.push.service.XMPushService", "com.xiaomi.push.service.XMJobService", "com.xiaomi.mipush.sdk.MessageHandleService");
            my %xgServiceMap;
            ...
            foreach my $service (@xgForceServiceList)
            {
                 for (my $i = 0; $i < $count; $i++)
                {
                    if ($serviceData->[$i]->{"android:name"} eq $service)
                    {
                        ...
                        if ($serviceData->[$i]->{"android:enabled"} ne "true")
                        {
                            printf "$service android:enabled must be true! \n";
                        }
                        last;
                    } else {
        ...
        my @xgForceReceiverList = ("com.xiaomi.push.service.receivers.NetworkStatusReceiver");
        my @xgForceReceiverIntentActionList = ("android.net.conn.CONNECTIVITY_CHANGE");
        checkReceiver($receiverData, @xgForceReceiverList, @xgForceReceiverIntentActionList);

        @xgForceReceiverList = ("com.xiaomi.push.service.receivers.PingReceiver");
        @xgForceReceiverIntentActionList = ("com.xiaomi.push.PING_TIMER");
        checkReceiver($receiverData, @xgForceReceiverList, @xgForceReceiverIntentActionList);

        @xgForceReceiverList = ("com.qq.xg4allDemo.receiver.MiMessageReceiver");
        @xgForceReceiverIntentActionList = ("com.xiaomi.mipush.RECEIVE_MESSAGE", "com.xiaomi.mipush.MESSAGE_ARRIVED", "com.xiaomi.mipush.ERROR");
        checkReceiver($receiverData, @xgForceReceiverList, @xgForceReceiverIntentActionList);
